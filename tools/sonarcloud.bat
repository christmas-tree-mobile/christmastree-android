...
IF NOT "%~1"=="" (
     ...
     && androiddeployqt.exe --input android-libchristmastree.so-deployment-settings.json --output android-build --android-platform "%ANDROID_PLATFORM%" --deployment bundled --gradle --no-gdbserver ^
     && CD .. ^
     && sonar-scanner.bat -D"sonar.projectKey=christmas-tree-mobile_christmastree-android" ^
                          -D"sonar.projectName=ChristmasTree Android" ^
                          -D"sonar.organization=christmas-tree-mobile-github" ^
                          -D"sonar.sources=." ^
                          -D"sonar.sourceEncoding=UTF-8" ^
                          -D"sonar.exclusions=3rdparty/**/*,qml/**/*,translations/*" ^
